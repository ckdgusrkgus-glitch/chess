...
- 사람이 백/흑 중 어느 쪽을 플레이할지는 게임 시작(또는 "새 게임") 때마다 무작위로 정해집니다. 흑을 배정받으면 보드가 자동으로 뒤집혀 내 기물이 항상 화면 아래쪽에 오도록 표시됩니다.
...
- 시작 화면의 "경기 복기" 버튼으로 진입합니다. 체크메이트/스테일메이트/무승부/기권으로 끝난 최근 20개 게임이 기기에 저장됩니다(SharedPreferences, 별도 서버 없음).
- 목록에서 게임을 선택하면 처음부터 한 수씩 "이전/다음"으로 넘겨볼 수 있고, 매 수마다 실제로 둔 수와 함께 최상급 AI가 그 자리에서 추천하는 수를 같이 보여줍니다.
...
app/src/main/java/chess/
  ...
  MoveNotation.kt         - Move <-> "e2e4" 같은 표기법 변환 (복기 저장/재생에 사용)
  ai/AiLevel.kt           - 난이도(레이팅/탐색 깊이/블런더 확률) 정의
  ai/Evaluator.kt         - 기물 가치 + 기물별 위치 테이블 평가 함수
  ai/ChessAi.kt           - 미니맥스 + 알파베타 + quiescence 탐색 엔진
  history/GameRecord.kt, GameRecordCodec.kt
                          - 완료된 게임 기록과 그 직렬화 (순수 Kotlin)
  history/GameHistoryStore.kt
                          - SharedPreferences에 최근 20게임을 저장/조회 (유일하게 Android 의존)
  ui/MainActivity.kt      - 시작 화면, AI 난이도 선택, 복기 화면 진입
  ui/GameActivity.kt      - 게임 화면: 색 랜덤 배정, AI 수 계산 트리거, 기보 기록, 게임 종료 시 저장
  ui/ChessBoardView.kt    - 보드를 직접 그리고 터치 입력을 처리하는 커스텀 View (보드 반전 지원)
  ui/HistoryActivity.kt   - 저장된 게임 목록
  ui/ReviewActivity.kt    - 한 수씩 복기 + AI 추천 표시
...
app/src/test/java/chess/
  ...
  MoveNotationTest.kt     - 기보 표기 <-> Move 왕복 변환 검증
  ai/ChessAiTest.kt       - AI가 항상 합법수를 두는지, 외통 기회를 놓치지 않는지 검증
  history/GameRecordCodecTest.kt - 게임 기록 직렬화 왕복 검증
```
